...
class CartController extends Controller
{
    ...
    public function singleAddToCartAjax(Request $request)
    {
        $request->validate([
            'slug'      =>  'required',
            // 'quant'      =>  'required',
        ...
        if ($product->stock < $quantity) {
            return response()->json(['error' => 'Out of stock, You can add other products.'], 400);
        }
        ...
        if ($already_cart) {
            ...
            // $already_cart->price = ($product->price * $quantity) + $already_cart->price ;
            $already_cart->amount = ($product->price * $quantity) + $already_cart->amount;

            if ($already_cart->product->stock < $already_cart->quantity || $already_cart->product->stock <= 0) return response()->json(['error' => 'Stock not sufficient!.'], 400);
